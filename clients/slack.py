...
class SlackClient:
    ...
    def add_message_reaction(self, channel: str, reaction: str, timestamp: str, dry_run: bool):
        if dry_run:
            logging.info(f"dry-run: adding reaction '{reaction}'"
                         f" to message [{timestamp}]")
            return True
        try:
            logging.info(f"adding reaction '{reaction}' to message [{timestamp}]")
            self.client.reactions_add(channel=channel, name=reaction, timestamp=timestamp)
            return True
        except SlackApiError as err:
            logging.warning(f"error reacting to message [{timestamp}]: {err}")
            raise
    ...
    def _get_conversation_history(self, channel: str, minutes: int, latest_ts: str = None):
        ...
        try:
            ...
            logging.warning(f"error loading conv. history: {err}")
            raise
    ...
    def get_conversation_history(self, channel: str, minutes: int):
        ...
        while history.get("has_more"):
            ...
            history = self._get_conversation_history(channel, minutes, last_ts)
            messages.extend(history["messages"])
    ...
    def _get_conversation_replies(self, channel: str, minutes: int, ts: str, latest_ts: str = None):
        ...
        try:
            ...
            logging.warning(f"error loading message replies: {err}")
            raise
