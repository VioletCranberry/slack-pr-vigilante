...
def get_arguments():
    ...
    parser.add_argument("--slack_time_window_minutes",
                        ...
                        type=int,
                        required=True,
                        env_var="SLACK_TIME_WINDOW_MINUTES")
    parser.add_argument("--slack_approved_reaction_name",
                        action="store",
                        type=str,
                        required=False,
                        default="white_check_mark",
                        env_var="SLACK_APPROVED_REACTION_NAME")
    parser.add_argument("--slack_merged_reaction_name",
                        action="store",
                        type=str,
                        required=False,
                        default="merged",
                        env_var="SLACK_MERGED_REACTION_NAME")
    parser.add_argument("--slack_github_error_reaction_name",
                        action="store",
                        type=str,
                        required=False,
                        default="sadpepe",
                        env_var="SLACK_GITHUB_ERROR_REACTION_NAME")
    ...
    parser.add_argument("--max_client_retries",
                        action="store",
                        type=int,
                        required=False,
                        default=5,
                        env_var="MAX_CLIENT_RETRIES")
